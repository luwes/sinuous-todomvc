(function () {
  ...
  f(() => localStorage.setItem('todos-sinuous', JSON.stringify(model))); // Make any signal jsonable by adding a toJSON method
  ...
  const editing = d$1();
  const filter = d$1('all');
  const completed = () => todos().filter(todo => todo.completed());
  const remaining = () => todos().filter(todo => !todo.completed());
  const displayed = () => filter() === 'all' ? todos() : filter() === 'completed' ? completed() : remaining();
  ...
  const TodoApp = () => m([m("header", {
    ...
    "class": "todo-count"
  }, m("strong", null, () => remaining().length), () => remaining().length === 1 ? ' item' : ' items', " left"), m("ul", {
    "class": "filters"
  }, m("li", null, m("a", {
    "class": () => cx({
      selected: filter() === 'all'
    ...
    href: "#/completed"
  }, "Completed"))), () => completed().length > 0 && m("button", {
